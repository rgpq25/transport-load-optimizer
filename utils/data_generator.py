...
def generateRandomDataset(outputPath):
    # random.seed(42)
    ...
    # SAVE
    save_dataset(clients, blocks, vehicles, routes, orders, outputPath)

if __name__ == "__main__":
    numExpDatasetPath = os.path.join(input_folder_path, "used_tests/numeric_experimentation")

    for i in range(0, 20):
        fileName = f"input{i + 1}.txt"
        outputPath = os.path.join(numExpDatasetPath, fileName)
        print(f"Generating dataset: {fileName}")
        generateRandomDataset(outputPath)